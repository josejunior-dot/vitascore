 ...
 *   localStorage.setItem("saluflow-api-url", "http://localhost:3333")
 */
...
export async function getApiUrl(): Promise<string | null> {
  return await getStore("saluflow-api-url");
}
...
export async function setApiUrl(url: string): Promise<void> {
  try {
    const { Preferences } = await import("@capacitor/preferences");
    await Preferences.set({ key: "saluflow-api-url", value: url });
  } catch {
    localStorage.setItem("saluflow-api-url", url);
  }
}
